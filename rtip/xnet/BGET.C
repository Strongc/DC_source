...
#define DIAG_SECTION_KERNEL DIAG_SECTION_BGET
...
static int (*compfcn) _((bufsize sizereq, int sequence)) = 0;
static PFVOID (*acqfcn) _((bufsize size)) = 0;
static void (*relfcn) _((PFVOID buf)) = 0;
...
PFVOID bget(bufsize requested_size)
{
    ...
    DEBUG_ASSERT(size > 0, "bget", NOVAR, 0, 0);
...
/*  while (b != &freelist) {   */
    while (b != freeptr) {
        ...
        if ((b->bh.bsize - size) > (SizeQ + (sizeof(struct bhead)))) {
            ...
            ba = BH(((PFBYTE) b) + (b->bh.bsize - size));
            bn = BH(((PFBYTE) ba) + size);
            DEBUG_ASSERT(bn->prevfree == b->bh.bsize, "bget", NOVAR, 0, 0);
...
#endif
            buf = (PFVOID) ((((PFBYTE) ba) + sizeof(struct bhead)));
#if (DEBUG_BGET)
            DEBUG_ERROR("bget: addr, len", DINT2, buf, requested_size);
#endif
            return buf;
        } else {
            struct bhead *ba;

            ba = BH(((PFBYTE) b) + b->bh.bsize);
            DEBUG_ASSERT(ba->prevfree == b->bh.bsize, "bget", NOVAR, 0, 0);

                    /* The buffer isn't big enough to split.  Give  the  whole
               shebang to the caller and remove it from the free list. */

            DEBUG_ASSERT(b->ql.blink->ql.flink == b, "bget", NOVAR, 0, 0);
            DEBUG_ASSERT(b->ql.flink->ql.blink == b, "bget", NOVAR, 0, 0);
...
#if (DEBUG_BGET)
    DEBUG_ERROR("bget: addr, len", DINT2, 0, requested_size);
#endif
    return 0;
}
...
PFVOID bgetz(bufsize size)
{
    ...
    DEBUG_ASSERT(rsize >= size, "bgetz:", NOVAR, 0, 0);
    V tc_memset(buf, 0, (MemSize) rsize);
    }
    return ((PFVOID) buf);
}
...
PFVOID bgetr(PFVOID buf, bufsize size)
{
    PFVOID nbuf;
    ...
    b = BH(((PFBYTE) buf) - sizeof(struct bhead));
    osize = -b->bsize;
#ifdef BECtl
    if (osize == 0) {
    ...
    bd = BDH(((PFBYTE) buf) - sizeof(struct bdhead));
    osize = bd->tsize - sizeof(struct bdhead);
    } else
#endif
    osize -= sizeof(struct bhead);
    DEBUG_ASSERT(osize > 0, "bgetr: ", NOVAR, 0, 0);
    V tc_movebytes((PFBYTE) nbuf, (PFBYTE) buf, /* Copy the data */
...
void brel(PFVOID buf)
{
...
#if (DEBUG_BGET)
    DEBUG_ERROR("brel: addr", DINT1, buf, 0);
#endif
    b = BFH(((PFBYTE) buf) - sizeof(struct bhead));
#ifdef BufStats
    ...
    bdh = BDH(((PFBYTE) buf) - sizeof(struct bdhead));
    DEBUG_ASSERT(b->bh.prevfree == 0, "brel", NOVAR, 0, 0);
#ifdef BufStats
    totalloc -= bdh->tsize;
    DEBUG_ASSERT(totalloc >= 0, "brel", NOVAR, 0, 0);
    numdrel++;            /* Number of direct releases */
#endif /* BufStats */
#ifdef FreeWipe
    V tc_memset((PFBYTE) buf, 0x55,
         (MemSize) (bdh->tsize - sizeof(struct bdhead)));
#endif /* FreeWipe */
#if (DEBUG_ASSERT_BGET)
    DEBUG_ASSERT(relfcn != 0, "brel: ", NOVAR, 0, 0);
#endif
    (*relfcn)((PFVOID) bdh);      /* Release it directly. */
    return;
    }
#endif /* BECtl */
...
#if (DEBUG_ASSERT_BGET)
    DEBUG_ASSERT(b->bh.bsize < 0, "brel: ", NOVAR, 0, 0);
#endif
...
#if (DEBUG_ASSERT_BGET)
    DEBUG_ASSERT(BH((PFBYTE) b - b->bh.bsize)->prevfree == 0, "brel: ",
        NOVAR, 0, 0);
#endif

#ifdef BufStats
    totalloc += b->bh.bsize;
    DEBUG_ASSERT(totalloc >= 0, "brel: totalloc > 0: ", DINT1, totalloc, 0);
#endif
    ...
    DEBUG_ASSERT(BH((PFBYTE) b - b->bh.prevfree)->bsize == b->bh.prevfree,
        "brel", NOVAR, 0, 0);
    b = BFH(((PFBYTE) b) - b->bh.prevfree);
    b->bh.bsize -= size;
    } else {
...
#if (DEBUG_ASSERT_BGET)
    DEBUG_ASSERT(freelist.ql.blink->ql.flink == &freelist, "brel: prev buffer not alloc", NOVAR, 0, 0);
#endif
    b->ql.flink = &freelist;
...
#if (DEBUG_ASSERT_BGET)
    DEBUG_ASSERT(BH((PFBYTE) bn + bn->bh.bsize)->prevfree == bn->bh.bsize,
        "brel: next buffer is free", NOVAR, 0, 0);
    DEBUG_ASSERT(bn->ql.blink->ql.flink == bn, "brel: ", NOVAR, 0, 0);
#endif
...
#ifdef FreeWipe
    V tc_memset(((PFBYTE) b) + sizeof(struct bfhead), 0x55,
        (MemSize) (b->bh.bsize - sizeof(struct bfhead)));
#endif
#if (DEBUG_ASSERT_BGET)
    DEBUG_ASSERT(bn->bh.bsize < 0, "brel: dh.bsize < 0", DINT1,
        bn->bh.bsize, 0);
#endif
...
#if (DEBUG_ASSERT_BGET)
    DEBUG_ASSERT(b->bh.prevfree == 0, "brel: ", NOVAR, 0, 0);
    DEBUG_ASSERT(BH((PFBYTE) b + b->bh.bsize)->bsize == ESent, "brel: ", NOVAR, 0, 0);
    DEBUG_ASSERT(BH((PFBYTE) b + b->bh.bsize)->prevfree == b->bh.bsize,
        "brel: ", NOVAR, 0, 0);
#endif
...
#if (DEBUG_ASSERT_BGET)
    DEBUG_ASSERT(numpblk == numpget - numprel, "brel: numpblk",
        DINT1, numpblk, 0);
#endif
#endif /* BufStats */
    }
...
void bectl(int (*compact) _((bufsize sizereq, int sequence)),
           PFVOID (*acquire) _((bufsize size)),
           void (*release) _((PFVOID buf)),
           bufsize pool_incr)
{
    compfcn = compact;
    acqfcn = acquire;
...
void bpool(PFVOID buf, bufsize len)
{
    struct bfhead *b = BFH(buf);
    struct bhead *bn;

#if (DEBUG_BGET)
    DEBUG_ERROR("bpool: commit addr, len", DINT2, buf, len);
#endif
#ifdef SizeQuant
    len &= ~(SizeQuant - 1);
#endif
    ...
    DEBUG_ASSERT(numpblk == numpget - numprel, "bpool: ", NOVAR, 0, 0);
#endif /* BufStats */
#endif /* BECtl */
    ...
    DEBUG_ASSERT(len - sizeof(struct bhead) <= -((bufsize) ESent + 1),
        "bpool: ", NOVAR, 0, 0);
    ...
    DEBUG_ASSERT(freelist.ql.blink->ql.flink == &freelist, "bpool: ", NOVAR, 0, 0);
    DEBUG_ASSERT(freelist.ql.flink->ql.blink == &freelist, "bpool: ", NOVAR, 0, 0);
    b->ql.flink = &freelist;
    b->ql.blink = freelist.ql.blink;
    freelist.ql.blink = b;
    b->ql.blink->ql.flink = b;
    ...
    DEBUG_ASSERT((~0) == -1, "bpool: ", NOVAR, 0, 0);
    bn->bsize = ESent;
}
            ...
            long *nget, long *nrel)
{
    ...
    *maxfree = -1;
    while (b != &freelist) {
        DEBUG_ASSERT(b->bh.bsize > 0, "bstats: ", NOVAR, 0, 0);
        *totfree += b->bh.bsize;
...
void bufdump(PFVOID buf)
{
    ...
    b = BFH(((PFBYTE) buf) - sizeof(struct bhead));
    DEBUG_ASSERT(b->bh.bsize != 0, "bufdump: ", NOVAR, 0, 0);
    if (b->bh.bsize < 0) {
        bdump = (PFBYTE) buf;
        bdlen = (-b->bh.bsize) - sizeof(struct bhead);
    } else {
        bdump = ((PFBYTE) b) + sizeof(struct bfhead);
    ...
    while (bdlen > 0) {
        ...
        for (i = 0; i < l; i++) {
            V tc_sprintf(bhex + i * 3, "%02X ", bdump[i]);
            bascii[i] = tc_isprint(bdump[i]) ? bdump[i] : ' ';
        }
        bascii[i] = 0;
        V tc_printf("%-48s   %s\n", bhex, bascii);
        bdump += l;
        bdlen -= l;
        while ((bdlen > 16) && (memcmp((PFBYTE) (bdump - 16),
                           (PFBYTE) bdump, 16) == 0)) {
            dupes++;
            bdump += 16;
            bdlen -= 16;
...
void bpoold(PFVOID buf, int dumpalloc, int dumpfree)
{
    struct bfhead *b = BFH(buf);
    ...
    while (b->bh.bsize != ESent) {
        ...
        if (bs < 0) {
            bs = -bs;
                V tc_printf("Allocated buffer: size %6ld bytes.\n", (long) bs);
            if (dumpalloc) {
                bufdump((PFVOID) (((PFBYTE) b) + sizeof(struct bhead)));
            }
        } else {
            PFBYTE lerr = "";

            DEBUG_ASSERT(bs > 0, "bpoold: ", NOVAR, 0, 0);
            if ((b->ql.blink->ql.flink != b) ||
            (b->ql.flink->ql.blink != b)) {
                lerr = "  (Bad free list links)";
            }
            V tc_printf("Free block:       size %6ld bytes.%s\n",
                        (long) bs, lerr);
#ifdef FreeWipe
            lerr = ((PFBYTE) b) + sizeof(struct bfhead);
...
int bpoolv(PFVOID buf)
{
    ...
    while (b->bh.bsize != ESent) {
        ...
        if (bs < 0) {
            ...
            DEBUG_ASSERT(bs > 0, "bpoolv: ", NOVAR, 0, 0);
            if (bs <= 0) {
                return 0;
            }
            if ((b->ql.blink->ql.flink != b) ||
                (b->ql.flink->ql.blink != b)) {
                V tc_printf("Free block: size %6ld bytes.  (Bad free list links)\n",
                         (long) bs);
                DEBUG_ASSERT(0, "bpoolv: ", NOVAR, 0, 0);
                return 0;
            }
#ifdef FreeWipe
            lerr = ((PFBYTE) b) + sizeof(struct bfhead);
...
static PFBYTE bchain = 0;         /* Our private buffer chain */
static PFBYTE bp = 0;         /* Our initial buffer pool */
...
static void stats(PFBYTE when)
{
    ...
    V tc_printf("  %ld direct gets, %ld direct frees\n", ndget, ndrel);
#endif /* BECtl */
}
...
#ifdef COMPACTRACE
    V tc_printf("Compaction requested.  %ld bytes needed, sequence %d.\n",
                (long) bsize, seq);
#endif

    if (protect || (seq > CompactTries)) {
#ifdef COMPACTRACE
        V tc_printf("Compaction gave up.\n");
#endif
        return 0;
    }
...
#ifdef COMPACTRACE
    V tc_printf("Compaction bailed out.\n");
#endif
#endif /* CompactTries */
    return 0;
}
...
static PFVOID bexpand(bufsize size)
{
    PFVOID np = 0;
    bufsize cural, totfree, maxfree;
    long nget, nfree;
    ...
    if (cural < PoolSize) {
        np = (PFVOID) malloc((unsigned) size);
    }
#ifdef EXPTRACE
    V tc_printf("Expand pool by %ld -- %s.\n", (long) size,
...
static void bshrink(PFVOID buf)
{
    if (((PFBYTE) buf) == bp) {
#ifdef EXPTRACE
        V tc_printf("Initial pool released.\n");
#endif
    bp = 0;
    }
#ifdef EXPTRACE
    V tc_printf("Shrink pool.\n");
#endif
    free((PFBYTE) buf);
}
...
int main(void)
{
    ...
    bp = malloc(ExpIncr);
    DEBUG_ASSERT(bp != 0, "main: ", NOVAR, 0, 0);
    bpool((PFVOID) bp, (bufsize) ExpIncr);
#else
    bp = malloc(PoolSize);
    DEBUG_ASSERT(bp != 0, "main: ", NOVAR, 0, 0);
    bpool((PFVOID) bp, (bufsize) PoolSize);
#endif

    stats("Create pool");
    V bpoolv((PFVOID) bp);
    bpoold((PFVOID) bp, dumpAlloc, dumpFree);

    for (i = 0; i < TestProg; i++) {
    PFBYTE cb;
    bufsize bs = pow(x, (double) (rand() & (ExpIncr - 1)));

    DEBUG_ASSERT(bs <= (((bufsize) 4) * ExpIncr), "main: ", NOVAR, 0, 0);
    bs = blimit(bs);
    if (rand() & 0x400) {
        cb = (PFBYTE) bgetz(bs);
    } else {
...
#endif
                newb = (PFBYTE) bgetr((PFVOID) fb, bs);
#ifdef BECtl
                protect = 0;
#endif
                if (newb != 0) {
