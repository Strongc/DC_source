...
*/
/*
 * pd67xx.c chip driver for the cirrus logic 6710, 6720 and intel 82365
...
KS_GLOBAL_CONSTANT byte mem_start_address_low[5]  = {0x10, 0x18, 0x20, 0x28, 0x30};
KS_GLOBAL_CONSTANT byte mem_start_address_high[5] = {0x11, 0x19, 0x21, 0x29, 0x31};
KS_GLOBAL_CONSTANT byte mem_end_address_low[5]  = {0x12, 0x1a, 0x22, 0x2a, 0x32};
KS_GLOBAL_CONSTANT byte mem_end_address_high[5]    = {0x13, 0x1b, 0x23, 0x2b, 0x33};
KS_GLOBAL_CONSTANT byte mem_offset_low[5]  = {0x14, 0x1c, 0x24, 0x2c, 0x34};
KS_GLOBAL_CONSTANT byte mem_offset_high[5] = {0x15, 0x1d, 0x25, 0x2d, 0x35};
KS_GLOBAL_CONSTANT byte mem_page[5] = {0x40, 0x41, 0x42, 0x43, 0x44};
KS_GLOBAL_CONSTANT byte io_start_address_low[2]  = {0x08, 0x0c};
KS_GLOBAL_CONSTANT byte io_start_address_high[2] = {0x09, 0x0d};
KS_GLOBAL_CONSTANT byte io_end_address_low[2]    = {0x0a, 0x0e};
KS_GLOBAL_CONSTANT byte io_end_address_high[2]   = {0x0b, 0x0f};
KS_GLOBAL_CONSTANT byte io_offset_low[2]         = {0x36, 0x38};
KS_GLOBAL_CONSTANT byte io_offset_high[2]        = {0x37, 0x39};

/* Table mapping logical socket number to chip/socket.
   There should be one pair per logical socket number */
KS_GLOBAL_CONSTANT byte chip_mask[2] = {0x0, 0x80};
KS_GLOBAL_CONSTANT byte socket_mask[2] = {0x0, 0x40};


RTIP_BOOLEAN pcmctrl_initted = FALSE;
...
/* bits for controlling the IO window characteristics. OR'ed together       */
...
/* bits for controlling the management interrupt OR'ed together       */
...
/* See bss.c     */
RTIP_BOOLEAN card_is_powered[NSOCKET];
int card_device_type[NSOCKET];

#define write_index_register(X) OUTBYTE(ADDR_INDEX,X)
#define read_data_register()    INBYTE(ADDR_DATA)
#define write_data_register(X)  OUTBYTE(ADDR_DATA,X)
...
/* Macro used to manipulte the second argument to calc io.
   Calc io's second argument is the pcmcia address that corresponds to
...
RTIP_BOOLEAN pcmctrl_init(void)  /* __fn__ */
{
int i;
byte mask;
    ...
    pcmcia_dummy();  /* Force linker to bring in pcmcia */
    ...
    for (i = 0; i < NSOCKET; i++)
        pcmctrl_card_down(i);
/*    comment out the following line for the ORiNOCO wireless card                         */
...
static void pcmctrl_enable_interrupt(int socket, int irq, RTIP_BOOLEAN on_off)
{
byte c;
...
void pcmctrl_map_linksys_regs(int socket, int irq, int iobase)  /*__fn__*/
{
    ...
    /* Allow bus lines (CS16) to determine transfer size                              */
    pd67xx_map_io(socket, IO_WINDOW_0, 32, (word)iobase, (byte)(IO_CTL_16BIT|IO_CTL_TR1));
    /* Turn on interrupts     */
    pcmctrl_enable_interrupt(socket, irq, TRUE);
...
void pcmctrl_map_ata_regs(int socket, IOADDRESS ioaddr, int interrupt_number)
{
    ...
    /* chip 0, Socket 0, window 0, size 16, isa_add = 0x170 pcmcia addr = 0x170       */
    /* Allow bus lines (CS16) to determine transfer size                              */
    pd67xx_map_io(socket, IO_WINDOW_0, 16, ioaddr,IO_CTL_AUTO);
...
/*                                                                              */
/* Type: void                                                                   */
/* Routine: pcmctrl_map_prism_regs                                              */
/* Description: Set up io windows, enable interrupts, power to Vpp              */
/* Input:                                                                       */
/* Output:                                                                      */
/*                                                                              */
/* ********************************************************************         */
...
void pcmctrl_map_prism_regs(int socket, int irq, int iobase)
{
    ...
    /*   isa_add = 0x170              */
    /*   pcmcia addr = 0x170          */
    /*   Allow bus lines (CS16)       */
    /*   to determine transfer        */
    /*   size                         */
    /* ------------------------       */
    pd67xx_map_io(socket, IO_WINDOW_0, 0x40, (word)iobase, (byte)(IO_CTL_AUTO|IO_CTL_16BIT|IO_CTL_TR1));
    ...
    /* ------------------------       */
    /*   Apply Vpp power (does        */
    /*   nothing since we use         */
    /*   auto Vpp)                    */
    /* ------------------------       */
    pcmctrl_device_power_up(socket);
...
/*  pcmctrl_card_is_installed(int socket, RTIP_BOOLEAN apply_power) - Check if a socket has a card installed      */
/*                                                                                                                */
...
RTIP_BOOLEAN pcmctrl_card_is_installed(int socket, RTIP_BOOLEAN apply_power)          /*__fn__*/
{
byte c;
...
/*  RTIP_BOOLEAN card_is_powered[socket] to TRUE.                                    */
/*                                                                                   */
/*                                                                                   */
/*  Inputs:                                                                          */
/*      Logical socket number                                                        */
...
RTIP_BOOLEAN pcmctrl_card_installed(int socket)          /*__fn__*/
{
...
RTIP_BOOLEAN pcmctrl_card_power_up(int socket)                       /*__fn__*/
{
byte c;
word i;
...
void pcmctrl_device_power_up(int socket)                        /*__fn__*/
{
    ...
    /* 0x377/0x3f7                                                                   */
    /* tbd - Try setting power                                                       */
     pd67xx_write(socket, 0x1E, 0x32);
...
/*  offset              - Offset into the CIS we're looking for                    */
/*                                                                                 */
/*  Outputs - Pointer to the memory window                                         */
/*                                                                                 */


PFBYTE  pd67xx_map(int socket , dword offset, RTIP_BOOLEAN is_cis)   /*__fn__*/
{
...
dword isa_cis_address;
    ...
    /* Pick the memory window we'll be looking through. - this is
...
#define W(INDEX, VALUE) pd67xx_write((int)socket,(int)(INDEX),(byte)(VALUE))
    ...
    phys_to_virtual( &p, isa_cis_address);
#undef W
    p = p + (int)pointer_offset;
    return (p);
}

PFBYTE  pd67xx_map_cis(int socket , dword offset)   /*__fn__*/
{
    return(pd67xx_map(socket , offset, TRUE));
}

PFBYTE  pd67xx_map_sram(int socket , dword offset)  /*__fn__*/
{
...
{
    pd67xx_index(socket,index);     /* Establish the register pointer */
    return((byte)read_data_register());
}
...
DEBUG_ERROR("CIS ADDRESS IS ", DINT1, (dword) p, 0);
    if (p)
    {
        for (j = 0; j < 8; j++)
        {
