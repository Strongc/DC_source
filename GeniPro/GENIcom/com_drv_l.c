...
#include "com_drv_l.h"            /* Access to itself                       */
...
#if (COM_IDLE_TYPE == TIMER_IDLE )
  #if(COM_IDLE_TIMER == SOFT)
    BIT COM_soft_timer_started;                            // flag used when timer idle is soft
  #endif
  static UCHAR geni_idle_counter;                             // counter used when timer idle is active
#elif (COM_IDLE_TYPE == GENI_IRQ_IDLE )
  BIT COM_soft_timer_started;                            // flag used when timer idle is soft
...
/*                                                                          */
/****************************************************************************/
#if(COM_IDLE_TYPE == HW_IDLE)
  static void InitHwIdle(void);
#elif(COM_IDLE_TYPE == TIMER_IDLE)
...
*---------------------------------------------------------------------------*/
void InitCOMDriver(UCHAR p_setup_param)
{
  UCHAR stop_bits = 0;
  UCHAR parity    = 0;
  UCHAR baudrate  = 0;
...
*---------------------------------------------------------------------------*/
  ...
  __interrupt void GeniCOMOutIrq(void)

#elif (defined K0)
  static GENI_OUT_IRQ GeniCOMOutIrq(void)

#elif ((defined MIPS) || (defined PC))
  void GeniCOMOutIrq(void)
...
*---------------------------------------------------------------------------*/
  ...
  __interrupt void GeniCOMInIrq(void)

#elif (defined K0)
  static GENI_IN_IRQ GeniCOMInIrq(void)
...
__interrupt void GeniCOMErrIrq(void)
#else
void GeniCOMErrIrq(void)
#endif

#elif (defined K0)
  static GENI_ERR_IRQ GeniCOMErrIrq(void)
...
*---------------------------------------------------------------------------*/
  ...
  __interrupt void GeniCOMLineIdleIrq(void)

#elif (defined K0)
  static GENI_IDLE_IRQ GeniCOMLineIdleIrq(void)

#elif ((defined MIPS) || (defined PC))
  void GeniCOMLineIdleIrq(void)
...
*---------------------------------------------------------------------------*/
    ...
    __interrupt void GeniCOMIdleTimerIrq(void)

  #endif
#elif (defined K0)
  GENI_IDLE_TIMER_IRQ GeniCOMIdleTimerIrq(void)
...
#if (COM_IDLE_PIN != GENI_NOT_USED)
/****************************************************************************
*     Name      :  GeniCOMLineBusyIrq                                       *
*     Inputs    :  none                                                     *
*     Outputs   :                                                           *
*     Updates   :  none                                                     *
*     Returns   :                                                           *
...
*---------------------------------------------------------------------------*/
  ...
  __interrupt void GeniCOMLineBusyIrq(void)

#elif (defined K0)
  static SOFT_IDLE_IRQ GeniCOMLineBusyIrq(void)

#elif ((defined MIPS) || (defined PC))
  void GeniCOMLineBusyIrq(void)
...
#endif // COM_IDLE_PIN != GENI_NOT_USED
