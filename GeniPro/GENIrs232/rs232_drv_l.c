...
/****************************************************************************/
/*                                                                          */
/* MODULE NAME      :    rs232_drv_l.c                                        */
/*                                                                          */
/* FILE NAME        :    rs232_drv_l.c                                        */
/*                                                                          */
...
#include "rs232_drv_if.h"           /* Access to Driver interface functions   */
#include "rs232_drv_l.h"            /* Access to itself                       */
...
#if (RS232_IDLE_TYPE == TIMER_IDLE )
  #if(RS232_IDLE_TIMER == SOFT)
    BIT RS232_soft_timer_started;                            // flag used when timer idle is soft
  #endif
  static UCHAR geni_idle_counter;                             // counter used when timer idle is active
#elif (RS232_IDLE_TYPE == GENI_IRQ_IDLE )
  BIT RS232_soft_timer_started;                            // flag used when timer idle is soft
  BIT RS232_geni_irq_idle;
#endif
...
/*                                                                          */
/****************************************************************************/
#if(RS232_IDLE_TYPE == HW_IDLE)
  static void InitHwIdle(void);
#elif(RS232_IDLE_TYPE == TIMER_IDLE)
...
*---------------------------------------------------------------------------*/
void InitRS232Driver(UCHAR p_setup_param)
{
  UCHAR stop_bits = 0;
  UCHAR parity    = 0;
  UCHAR baudrate  = 0;
...
*---------------------------------------------------------------------------*/
void TransmitRS232Byte(UCHAR tx_byte)
{
  GENI_TXD = tx_byte;
}

/****************************************************************************
*     Name      : GeniRS232OutIrq                                             *
*     Inputs    :                                                           *
*     Outputs   :                                                           *
*     Updates   :                                                           *
*     Returns   :                                                           *
...
*---------------------------------------------------------------------------*/
  ...
  __interrupt void GeniRS232OutIrq(void)

#elif (defined K0)
  static GENI_OUT_IRQ GeniRS232OutIrq(void)

#elif ((defined MIPS) || (defined PC))
  void GeniRS232OutIrq(void)
...
#endif
{
  ...
  {
     preamble = FALSE;
     TransmitRS232Byte(PLM_2ND_PREAMBLE);
  }
  else
  #endif
  {
...
*---------------------------------------------------------------------------*/
  ...
  __interrupt void GeniRS232InIrq(void)

#elif (defined K0)
  static GENI_IN_IRQ GeniRS232InIrq(void)

#elif ((defined MIPS) || (defined PC))
  void GeniRS232InIrq(void)
...
__interrupt void GeniRS232ErrIrq(void)
#else
void GeniRS232ErrIrq(void)
#endif

#elif (defined K0)
  static GENI_ERR_IRQ GeniRS232ErrIrq(void)
...
*---------------------------------------------------------------------------*/
  ...
  __interrupt void GeniRS232LineIdleIrq(void)

#elif (defined K0)
  static GENI_IDLE_IRQ GeniRS232LineIdleIrq(void)

#elif ((defined MIPS) || (defined PC))
  void GeniRS232LineIdleIrq(void)
...
#if ((defined V850E) || (defined V850ES))
  #if ( RS232_IDLE_TIMER == SOFT )
    void GeniRS232IdleTimerIrq(void)

  #else
    #pragma vector = GENI_IDLE_TIMER_IRQ
    __interrupt void GeniRS232IdleTimerIrq(void)

  #endif
#elif (defined K0)
  GENI_IDLE_TIMER_IRQ GeniRS232IdleTimerIrq(void)
...
#if (RS232_IDLE_PIN != GENI_NOT_USED)
/****************************************************************************
*     Name      :  GeniRS232LineBusyIrq                                       *
*     Inputs    :  none                                                     *
*     Outputs   :                                                           *
*     Updates   :  none                                                     *
*     Returns   :                                                           *
...
*---------------------------------------------------------------------------*/
  ...
  __interrupt void GeniRS232LineBusyIrq(void)

#elif (defined K0)
  static SOFT_IDLE_IRQ GeniRS232LineBusyIrq(void)

#elif ((defined MIPS) || (defined PC))
  void GeniRS232LineBusyIrq(void)
...
#endif // RS232_IDLE_PIN != GENI_NOT_USED
...
*     Inputs    :  none                                                     *
*     Outputs   :                                                           *
*     Updates   :  RS232_soft_timer_started, channel_busy, RS232_geni_irq_idle  *
*     Returns   :                                                           *
...
void GeniRS232IdleTimerIrq(void)
{
  if (RS232_soft_timer_started == TRUE)
  {
    if (RS232_geni_irq_idle == TRUE)                              // we didn't receive any since last time
    {
      RS232_soft_timer_started = FALSE;                           // Stop the checking
      channel_busy = FALSE;                                     // indicate that the channel is busy
      CHANNEL_BUSY(CH_NO, channel_busy);                        // signal that the channel is busy
    }
    RS232_geni_irq_idle = TRUE;
  }
}
#endif
